# 문서 가이드 모음

이 폴더에는 VSS 프로젝트의 모든 가이드 문서가 포함되어 있습니다.

## 📚 문서 목록

### 배포 가이드

1. **[DEPLOYMENT_GUIDE.md](./DEPLOYMENT_GUIDE.md)**
   - Vue 애플리케이션 배포 가이드
   - Vercel, Netlify, GitHub Pages 등 다양한 배포 옵션
   - Nginx, Apache, Docker 배포 방법 포함

### 설정 가이드

2. **[INTELLIVIX_SMTP_SETUP.md](./INTELLIVIX_SMTP_SETUP.md)**
   - @intellivix.ai 도메인용 SMTP 설정 가이드
   - Google Workspace, Microsoft 365, 자체 메일 서버 설정 방법

3. **[SMTP_SETUP_GUIDE.md](./SMTP_SETUP_GUIDE.md)**
   - 일반적인 SMTP 설정 가이드
   - Gmail, Outlook, Naver, Daum 등 다양한 이메일 서비스 설정 방법

## 🔍 빠른 찾기

### 프론트엔드 배포하기
→ [DEPLOYMENT_GUIDE.md](./DEPLOYMENT_GUIDE.md)

### 이메일 인증 설정하기
→ [SMTP_SETUP_GUIDE.md](./SMTP_SETUP_GUIDE.md) 또는 [INTELLIVIX_SMTP_SETUP.md](./INTELLIVIX_SMTP_SETUP.md)

## 📝 관련 문서

- **SQL 스크립트 가이드**: `../sql/README.md`
- **프로젝트 메인 README**: `../README.md`

## 💡 팁

- 배포 전에 모든 가이드를 읽어보시기 바랍니다
- 문제가 발생하면 각 가이드의 "문제 해결" 섹션을 참고하세요
- 프로덕션 환경에서는 보안 설정을 반드시 확인하세요

