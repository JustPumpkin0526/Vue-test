...
export function useVssUi() {
  ...
  const selectedLabel = computed(() => options.find(o => o.value === selected.value)?.label ?? 'Select Chunk Size')
  function toggleDropdown(){ open.value = !open.value }
  function selectOption(option){ selected.value = option.value; open.value = false }
  function onDocClick(e){ if (dropdownRef.value && !dropdownRef.value.contains(e.target)) open.value = false }
  ...
  const toggleAccordion = (i) => { accordionItems[i].open = !accordionItems[i].open }

  const settings = ref({ enableChat: true, enableHistory: true })
  const question = ref('')
  const response = ref('')
  ...
  const handleFileUpload = (event) => {
    ...
    if (videoFile) { files.value.push(videoFile); videoUrl.value = URL.createObjectURL(videoFile) }
    else alert('동영상 파일만 업로드할 수 있습니다.')
  }
  const handleDrop = (e) => {
  ...
  // Submit VSS
  const VSS_API_URL = 'http://localhost:7100/gradio_api/queue/join'
  const submitFiles = async () => {
    if (!files.value.length) return alert('Please upload a video file first.')
  ...
  function openParams(){ isParamOpen.value = true; document.documentElement.style.overflow = 'hidden' }
  function closeParams(){ isParamOpen.value = false; document.documentElement.style.overflow = '' }
  function applyParams(){ /* TODO: 서버 반영 */ closeParams() }

  function onKeydown(e){ if (e.key === 'Escape' && isParamOpen.value) closeParams() }
